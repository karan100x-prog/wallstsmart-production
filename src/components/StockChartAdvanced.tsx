...
const StockChartAdvanced: React.FC<StockChartAdvancedProps> = ({ symbol }) => {
  const [selectedRange, setSelectedRange] = useState<TimeRange>('5Y'); // Default to 5Y
  ...
  // Enhanced tooltip with smart volume formatting
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload[0]) {
      const data = payload[0].payload;
      
      // Smart volume formatting
      const formatVolumeTooltip = (volume: number) => {
      ...
      return (
        <div className="bg-gray-900 border border-gray-700 rounded p-2 text-sm">
          ...
          <p className="text-gray-300 text-xs mt-1">
            Volume: {formatVolumeTooltip(data.volume)}
          </p>
          {data.isAdjusted && (
