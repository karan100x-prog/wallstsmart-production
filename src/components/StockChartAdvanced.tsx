...
function adjustPricesForSplits(data: any[], symbol: string): any[] {
  ...
  return data.map((item) => {
    ...
    for (const split of splits) {
      ...
      if (splitDate > itemDate) adjustmentFactor *= split.ratio;
    }
    ...
    return {
      ...item,
      close: closePrice,
      adjustedClose: closePrice / adjustmentFactor,
      volume,
...
const StockChartAdvanced: React.FC<StockChartAdvancedProps> = ({ symbol }) => {
  ...
  const formatDateSmart = (
    ...
    if (range === '6M') return date.toLocaleDateString('en-US', { month: 'short' });
    if (range === '1Y')
      ...
      return (
        <div className="bg-gray-900 border border-gray-700 rounded p-2 text-sm">
          ...
          {showVolume && data.volume && (
            <p className="text-blue-400">Vol: {formatVolume(data.volume)}</p>
          )}
          {data.isAdjusted && (
  ...
  const hasSplits =
    STOCK_SPLITS[symbol.toUpperCase()] &&
    STOCK_SPLITS[symbol.toUpperCase()].length > 0;
          ...
          <div className="w-full">
            <ResponsiveContainer width="100%" height={300} style={{ minWidth: 0 }}>
              <ComposedChart data={chartData}>
                ...
                {showVolume && (
                  <Bar yAxisId="volume" dataKey="volume" fill="#60A5FA" opacity={0.3} />
                )}
                <Line
                  yAxisId="price"
                  type="monotone"
