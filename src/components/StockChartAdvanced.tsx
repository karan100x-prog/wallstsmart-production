// StockChartAdvanced.tsx - Restored Working Version
...
const StockChartAdvanced: React.FC<StockChartAdvancedProps> = ({ symbol }) => {
  ...
  const loadChartData = async () => {
    ...
    try {
      ...
      if (data?.['Time Series (Daily)']) {
        ...
        const availableDays = Math.min(daysToShow, dates.length);
        const filteredDates = dates.slice(-availableDays);
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-gray-900 rounded-lg">
        <div className="text-center">
          <p className="text-gray-400 mb-2">No data available for {timeRange}</p>
  ...
  const priceChange = lastDataPoint && firstDataPoint 
    ? ((lastDataPoint.price - firstDataPoint.price) / firstDataPoint.price * 100)
    : 0;
